...
Teclas::Teclas(const ros::NodeHandle &nh_):nh(nh_)
{
  ...
  gradient_image_pub = nh.advertise<sensor_msgs::Image>("gradient_image",3);
...
bool Teclas::SaveMap2txt(std_srvs::SetBool::Request& req,std_srvs::SetBool::Response& res)
{
  ...
  std::ofstream file("/home/r/Mysoftware/Paper_Expriment_Simulation/Wheel_uneven_handle/plane-oct-data.txt");
  ...
  std::ofstream file_point("/home/r/Mysoftware/Paper_Expriment_Simulation/Wheel_uneven_handle/elevation-points.txt");
  for(int i=0;i<allpoints.size();i++)
  {
    if (file_point.is_open()) 
    {
            file_point << std::fixed << std::setprecision(3) << allpoints.points[i].x;
            file_point << " ";
            file_point << std::fixed << std::setprecision(3) << allpoints.points[i].y;
            file_point << " ";
            file_point << std::fixed << std::setprecision(3) << allpoints.points[i].z;
            file_point << "\n";
    }
  }
      file_point.close();
...
bool Teclas::ShowSubspaceInfor(std_srvs::SetBool::Request& req,std_srvs::SetBool::Response& res)
{
  ...
  for(int j=0;j<=31;j++)
  {
    ...
    if(subspace_array[i][j]->state==sub_space::EXPLORING)
    {
      ...
      tt_f_cct += subspace_array[i][j]->frontiers.size();
    }
    ...
    std::cout<<"各子空间的Frontier数量和为"<<tt_f_cct<<std::endl;
...
//quaternion = (r*sin(Theta/2),cos(Theta/2))
  ...
  int attc=0;
  int free_cct=0;
  std::ofstream file_effi("/home/r/Mysoftware/Paper_Expriment_Simulation/Wheel_uneven_handle/efficiency.txt",std::ios::app);
  for(f_map::FrontierOcTree::leaf_iterator it = m_octree->begin_leafs(m_octree->getTreeDepth()), end = m_octree->end_leafs();it != end; ++it)
  {
    attc++;free_cct++;
...
void Teclas::PublishFrontierMarker()
{
  ...
  std::cout<<"Frontier的总数量为 "<<frontier.size()<<std::endl;
...
//返回<0,表示平面不可穿行，返回>=0，表示散点到平面的距离绝对值之和。这里返回的内点比例一定是>=0.2的，即便是UNTRA也是这样
int Teclas::fit_plane_to_cloud(pcl::ModelCoefficients::Ptr coefficients,const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,const double dist_thes,float& precent_inline,float& varia)
{
  ...
  // std::cout<<"fit_plane_to_cloud2"<<std::endl;
    ...
    if(precent_inline<=0.2)precent_inline=0.2;
...
void Teclas::PrepareServiceMsg_WholeMap(srvbg::getlcplan::Response* temprqst,int st_sub1,int st_sub2,float posx,float posy,float posz)
{
  ...
  std::vector<int> obs_list;
	...
	float sub2d_startz = posz-1.0;
	float sub2d_endz = posz+1.0;
